...
import { Upload, FileVideo, CheckCircle, Play, Settings, AlertCircle, ArrowLeft, Loader2, Sparkles, Languages, FileAudio, Cloud, Server, Key, Terminal, Download, Copy, ExternalLink, Youtube, Link } from 'lucide-react';
  ...
  const processFile = async () => {
      ...
      try {
          ...
          if (mode === 'cloud') {
              ...
              // Guard against massive files causing browser crash on FileReader
              if (file.size > 500 * 1024 * 1024) { // 500MB Limit for browser handling
              ...
              // In a real local setup, we would POST the file to the local endpoint
              // await fetch(`${localServerUrl}/transcribe`, { method: 'POST', body: formData });
              
              // For now, simulate error if no server exists
              setTimeout(() => {
                  setStatus('error');
